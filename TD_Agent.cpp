...
void TD_Agent<State_T>::findAction2()
{
	...
	std::vector<Action> possible_actions = m_environment->possible_actions(m_self_pointer);
		...
		else
		{
			...
			Action nextAction = TD_Agent<State_T>::nextAction(possible_actions);
			auto states = m_environment->assume_action(m_self_pointer, actual_state, nextAction);
			auto nextReward = m_environment->reward(m_self_pointer, states[0]);
			double errorSignal;

			if(lastReward < reward)
				errorSignal = learnRate * (0.9 + (nnetwork.getValue(nextPerception) - nnetwork.getValue(lastPerception)));
			else
				errorSignal = learnRate * ((nnetwork.getValue(nextPerception) - nnetwork.getValue(lastPerception)));
			nnetwork.backProb(errorSignal, 1.0);
			lastPerception = nextPerception;
			...
			if (nextReward > reward)
			{
				...
				errorSignal = learnRate * (0.1 - nnetwork.getValue(lastPerception));
				nnetwork.backProb(errorSignal, 1.0);
			}
			lastReward = reward;
			m_environment->apply_action(m_self_pointer, nextAction);
		}
}
